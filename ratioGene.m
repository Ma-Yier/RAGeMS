function [x_target,alpha,knockout] = ratioGene(model,id_biomass,id_target,TMGR,max_loop,gap,numMultiStrat)
    ...
    if ~isempty(pool)
        ...
        for j=1:size(pool,2)
            ...
            if size(x,1)==n+nGpr+nAux+nGen
                [x_target,knockout_gene]=verifyGeneKnock(model,x(n+nGpr+nAux+1:n+nGpr+nAux+nGen,:),indGPR,id_biomass,id_target,1,0.05);
